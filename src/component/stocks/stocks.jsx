...
const Stocks = () => {
  const { contentLoading, queryStatus, isMerge} = useSelector((state) => state.mutualStateReducer)
  const { allStocks, unSaleStocks, showStocks } = useSelector((state) => state.apiDataReducer)
  ...
  return (
    <div>
      ...
      <table className="table table-dark" style={{whiteSpace: 'nowrap'}}>
        ...
        <tr>
          <th scope="col">#</th>
          { !isStockHistory && isMerge === false &&<th scope="col">賣出</th>}
          { !isStockHistory && isMerge === false && <th scope="col">購買日期</th>}
        ...
        <tbody>
        {
          !contentLoading && stocks !== 'No Data' && (stocks.length !== 0) && stocks.map((stock, index) => (
          <Stock key={stock.number+index} stock={stock} index={index+1} isMerge = {isMerge}/>
          ))
        }
